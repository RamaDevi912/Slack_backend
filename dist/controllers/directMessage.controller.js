...
export const getOrCreateConversation = async (req, res, next) => {
    try {
        ...
        const room = await directMessageService.getOrCreateRoom(workspaceId, req.user.id, recipientId);
        res.status(200).json({
            success: true,
...
export const sendMessage = async (req, res, next) => {
    try {
        ...
        const message = await directMessageService.sendMessage(roomId, req.user.id, receiverId, content);
        res.status(201).json({
            success: true,
...
export const getMessageHistory = async (req, res, next) => {
    try {
        ...
        const messages = await directMessageService.getMessages(roomId, req.user.id, {
            page: parseInt(page),
...
export const getUserConversations = async (req, res, next) => {
    try {
        ...
        const rooms = await directMessageService.getUserRooms(workspaceId, req.user.id);
        res.status(200).json({
            success: true,
            data: rooms,
        });
 ...
 * Update DM Message
 */
export const updateDirectMessage = async (req, res, next) => {
    try {
        const { messageId } = req.params;
 ...
 * Add DM Reaction
 */
export const addDMReaction = async (req, res, next) => {
    try {
        const { messageId } = req.params;
 ...
 * Remove DM Reaction
 */
export const removeDMReaction = async (req, res, next) => {
    try {
        const { messageId } = req.params;
        const { emoji } = req.body;
        const userId = req.user?.id;
